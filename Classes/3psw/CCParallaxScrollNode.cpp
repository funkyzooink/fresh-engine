...
void CCParallaxScrollNode::updateWithVelocity(Point vel, float dt)
{
    ...
    {
        CCParallaxScrollOffset* scrollOffset = _scrollOffsets.at(i);
        Point relVel = scrollOffset->getRelVelocity() * PTM_RATIO;
        Point totalVel = vel + relVel;
        Point totalDelta = (totalVel * dt);
        Point offset = Vec2(totalDelta.x * scrollOffset->getRatio().x, totalDelta.y * scrollOffset->getRatio().y);
...
void CCParallaxScrollNode::addInfiniteScrollWithObjects(const cocos2d::Vector<Sprite*>& objects, int z, Point ratio,
                                                        Point pos, Point dir, Point relVel, Point padding)
{
...
void CCParallaxScrollNode::addInfiniteScrollWithObjects(const cocos2d::Vector<Sprite*>& objects, int z, Point ratio,
                                                        Point pos, Point dir, Point relVel)
{
...
void CCParallaxScrollNode::addInfiniteScrollXWithZ(int z, Point ratio, Point pos, Sprite* firstObject, ...)
{
    ...
    for (Sprite* arg = firstObject; arg != nullptr; arg = va_arg(args, Sprite*))
    {
